# SysConst CLI — LLM Generation & Versioning

Technical specification for adding LLM-powered generation and Git-based versioning to the SysConst CLI.
...
2. **LLM Generation** — Generate and evolve SysConst specifications using AI models
...
When `sysconst init` is run, it creates the following structure:

```
<project-name>/
├── .git/                          # Git repository (auto-initialized)
│   └── ...
├── .sysconst/                      # SysConst project metadata
│   ├── config.yaml                # Project-level configuration
│   └── cache/                     # Generation cache (gitignored)
├── <project-name>.sysconst.yaml    # Main specification file
├── .gitignore                     # Pre-configured gitignore
└── README.md                      # Auto-generated project readme
```

### 1.3 CLI Command: `sysconst init`

```bash
sysconst init [project-name] [options]
...
Examples:
  ...
  sysconst init myshop -d "E-commerce platform with orders and payments"
  
  # Create in current directory
  sysconst init -d "Task management system"
  
  # Create minimal project without LLM
  sysconst init myapp --no-generate
  
  # Non-interactive mode
  sysconst init myapp -d "Blog platform" -y
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│     sysconst init myshop -d "E-commerce with orders and payments"        │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
                         ┌─────────────────────┐
...
#### `.sysconst/config.yaml`

```yaml
# SysConst Project Configuration
# Generated by: sysconst init

project:
  name: myshop
  specFile: myshop.sysconst.yaml
...
SysConst-managed project.

## Specification

- **Spec file**: `myshop.sysconst.yaml`
...
sysconst validate myshop.sysconst.yaml

# Evolve specification
sysconst evolve myshop.sysconst.yaml -c "Add new feature"

# View version history
sysconst history

# Bump version
sysconst version bump minor -m "Description"
```
...
When running `sysconst init` in a directory that already has files:
...
2. **If `.sysconst/` exists** — Error: "Project already initialized"
3. **If `*.sysconst.yaml` exists** — Error: "Spec file already exists"
...
# ✓ Created myapp/
# ✓ Created myapp/.sysconst/config.yaml
# ✓ Created myapp/myapp.sysconst.yaml
...
- `sysconst version bump` updates spec only (no commit/tag)
- `sysconst history` shows only spec history (no Git info)
- `sysconst diff` uses spec history only
- `sysconst checkout` is disabled
...
```
$ sysconst init myshop -d "E-commerce platform"

SysConst Project Initialization
==============================
...
Next steps:
  cd myshop
  sysconst validate myshop.sysconst.yaml
  sysconst evolve myshop.sysconst.yaml -c "Add feature"
```
...
- Generate valid SysConst specifications from natural language descriptions
...
┌─────────────────────────────────────────────────────────────────┐
│                         CLI Commands                             │
│  sysconst generate | sysconst evolve | sysconst chat               │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
Generate a new SysConst specification from a description.
...
Examples:
  sysconst generate "E-commerce with orders and payments" -o shop.sysconst.yaml
  sysconst generate "Task management system" --provider ollama --model llama3
```
...
```bash
sysconst evolve <spec-file> [options]

Arguments:
  spec-file            Path to existing SysConst file
...
Examples:
  sysconst evolve app.sysconst.yaml -c "Add phone field to customer"
  sysconst evolve app.sysconst.yaml -c "Add notification system" --bump major
```
...
```bash
sysconst chat [spec-file] [options]
...
Examples:
  sysconst chat                           # Start new spec interactively
  sysconst chat app.sysconst.yaml          # Modify existing spec
```
...
```
~/.sysconst/config.yaml       # Global config
./.sysconst.yaml              # Project config (overrides global)
```
...
```markdown
Generate an SysConst DSL v1 specification for the following system:
...
1. Generate a complete, valid SysConst YAML
...
```markdown
Evolve the following SysConst specification based on the change request.
...
- Synchronize SysConst semver with Git commits and tags
...
```bash
sysconst version [command] [options]
...
Examples:
  sysconst version                                    # Show: 1.2.0
  sysconst version bump minor -m "Add notifications"
  sysconst version bump patch -m "Fix customer email" \
    -c "add-field:entity.customer:emailVerified:bool"
  sysconst version check
  sysconst version tag
```
...
Examples:
  sysconst history
  sysconst history --git --changes
  sysconst history -n 5 --json
```
...
```bash
sysconst diff <version1> <version2> [options]
...
Examples:
  sysconst diff 1.0.0 1.2.0
  sysconst diff v1.1.0              # Compare v1.1.0 to current
  sysconst diff HEAD~1              # Compare to previous commit
```
...
```bash
sysconst checkout <version> [options]
...
Examples:
  sysconst checkout 1.0.0
  sysconst checkout v1.1.0 --branch feature/from-v1.1
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│              sysconst version bump minor -m "message"             │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
                    ┌──────────────────┐
...
When `sysconst init myproject` is run:
...
│       └── .gitkeep
├── myproject.sysconst.yaml          # Main specification (v1.0.0)
...
| `PROJECT_EXISTS` | `.sysconst/` already exists | Project already initialized |
| `SPEC_FILE_EXISTS` | `*.sysconst.yaml` already exists | Remove or rename existing spec |
...
| Error | Cause | Resolution |
|-------|-------|------------|
| `NOT_GIT_REPO` | No .git directory | Run `sysconst init` or `git init` |
...
- [ ] `sysconst init` creates complete project structure
...
- [ ] `sysconst generate` creates valid spec from description
- [ ] `sysconst evolve` updates spec with proper history
...
- [ ] `sysconst version bump` updates spec + commits + tags
- [ ] `sysconst history` shows version history
- [ ] `sysconst diff` compares versions
- [ ] `sysconst version check` validates consistency
- [ ] Works without Git (graceful degradation with warnings)

### Integration

- [ ] `sysconst init` + LLM generation in single command
- [ ] `sysconst evolve` + version bump in single command
- [ ] Generated specs pass all 6 validation phases
- [ ] Error messages are actionable
- [ ] All commands respect `.sysconst/config.yaml`
...
- `sysconst migrate` — Generate migration code
- `sysconst codegen` — Generate code from spec
- `sysconst publish` — Publish to registry
- `sysconst import` — Import from OpenAPI/GraphQL
